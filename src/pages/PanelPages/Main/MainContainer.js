...
class MainContainer extends React.Component {
  ...
  componentDidUpdate() {
    if (this.state.page !== this.props.page) {
      ...
      if (
        this.state.menu.find((x) => x.text === this.props.page) !== undefined
      ) {
        this.setState({
          pageIndex: this.state.menu.findIndex(
            (x) => x.text === this.props.page
          ),
        });
      }
    }
    if (this.state.subPage !== this.props.subPage) {
      ...
      if (
        ...
        this.state.menu.find((x) => x.text === this.state.page).subMenu.length >
          0
      ) {
        this.setState({
          subPageIndex: this.state.menu
            .find((x) => x.text === this.state.page)
            .subMenu.findIndex((x) => x.text === this.props.page),
        });
      }
    }
  }
